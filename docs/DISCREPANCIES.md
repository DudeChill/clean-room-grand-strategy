...
| MIL-021 | MAJOR | Military | Divisions upgrade to newer equipment models (the reference lets a battalion slot draw any allowed variant, so a new model reaches the field) | Division slots hold one exact EquipmentId: `reinforce_division` requires the template to name that equipment, and division demand is keyed on it, so a newly produced model only reaches new units, never the existing army, and the AI cannot retrofit. Measured in v0.7.0: VEL's `VEL_infantry_1` line built 186,270 units, but no division switched its equipment. Design exists: give a BattalionSlot a family/variant set (or match on EquipmentDef::archetype with a preference order), teach compute_equipment_demand to aggregate demand per family, and let the AI re-point templates at its own designs | OPEN (blocker for 'research -> new model -> better divisions') |
