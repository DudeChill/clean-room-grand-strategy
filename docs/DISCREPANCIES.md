...
| INT-001 | MAJOR | Intelligence | Agencies, networks, operations, decryption | Closed in v0.10.0: 13 agency upgrades (network growth, operation speed, crypto speed, counter-intelligence, chained and PP-gated), 10 operations across all six kinds, networks with growth/decay/exposure and burn risk, decryption capped below certainty, and two battle effects (attack bonus from intel, planning denial from broken ciphers) which are zero when the attacker knows nothing. Measured on the shipped scenario, 365 days, seed 4242: 7/7 countries own an upgrade, 10 networks mean strength 64.1, operations in flight steal_tech 4 / sabotage_industry 3, and the implementing agent measured 12 + 6 + 4 + 4 completions by kind with burns | CLOSED (residuals: ideology_shift applies as war support, no counter-intel modifier kind) |
...
| MOD-004 | MINOR | Modding | Mods can override the scenario and map | `load_content` resolves `data/common/*` only, so a mod shipping `scenarios/*.json` or `maps/*.json` is rejected as an unknown content file; overriding the starting scenario needs `load_scenario` (src/data/scenario.cpp) to take mod roots the way `load_content` now does. Everything else in the mod layer is closed | OPEN |
| MOD-002 | MODERATE | Modding | Deterministic mod load order with override diagnostics | Closed in v0.9.0: `--mods <root[,root...]>` discovers `<root>/<mod>/mod.json`, orders mods by dependencies then `load_after` hints with ties broken by name (cycles rejected and named), merges tables after the base content, replaces entries by key and appends `add:`-marked ones, and prints a deterministic load report; a mod that fails validation is skipped as a whole so a bad mod cannot leave half-state. `--validate-content` runs the same code path and exits non-zero on any diagnostic. Verified live: `--mods data/mods --serve` and `/api/state` contained the example mod's `assault_rifles_1`; a broken mod refuses to start instead of silently loading without it | CLOSED |
| MOD-003 | MINOR | Modding | Mod test pack | Closed in v0.9.0: `tests/mods/` holds eight fixtures (clean override, dependency chain, load_after hint, `add:` append, malformed file, unknown table, unknown key, conflict) with a README line each; `scripts/validate-content.sh` covers 13 cases and is wired into the release gate; `tests/test_modpack.cpp` adds 10 tests (268 total); `docs/MODDING.md` documents manifests, load order, override rules and packaging | CLOSED |
